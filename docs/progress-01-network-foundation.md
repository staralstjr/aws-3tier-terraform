...
이 단계에서는 서브넷까지만 만들었고 **인터넷과 연결되는 경로가 없었다.**
이후 아래를 순차적으로 구축해 3-tier를 완성했다.

- [x] **Internet Gateway + 라우팅 테이블** — public 서브넷에 인터넷 경로 부여
  → [concept-routing-igw.md](concept-routing-igw.md)
- [x] **NAT Gateway** — private(App) 서브넷의 아웃바운드 전용 출구
  → [concept-nat-gateway.md](concept-nat-gateway.md)
- [x] **EC2 + Auto Scaling Group + ALB** — App tier 서버 무리 및 트래픽 분산
  → [concept-app-tier-alb.md](concept-app-tier-alb.md)
- [x] **RDS + Security Group** — DB tier 격리
  → [concept-db-tier-rds.md](concept-db-tier-rds.md)

---

*참고: NAT Gateway는 "private 위치의 서버가 노출 없이 고정 IP로 외부에 나간다"는,
온프레미스에서 프록시 서버로 풀던 문제와 동일한 역할을 한다.
→ [concept-nat-gateway.md](concept-nat-gateway.md)*